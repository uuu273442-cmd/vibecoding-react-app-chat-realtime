...
// COMPLETE FINAL - Upload + Search + Pin + Cursor Pagination
...
export default function ChatWindow({ conversation }) {
  ...
  // Update pinned messages khi messages thay đổi
  useEffect(() => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      // Response: { messages: [], nextCursor: "id", hasMore: bool }
  ...
  const loadMoreMessages = async () => {
    ...
    try {
      ...
      // Gọi API với before = nextCursor (messageId cũ nhất hiện tại)
      const data = await getMoreMessages(conversation._id, nextCursor, 19);

      // Prepend messages cũ hơn lên đầu danh sách
  ...
  const handleMessageDeleted = (data) => {
    ...
    setMessages(prev => prev.map(msg => {
      ...
      if (scope === 'everyone') {
        return { ...msg, content: 'Message deleted', isDeleted: true };
      } else {
    ...
    setMessages(prev => {
      ...
      if (links[0].messageId === lastMessage._id) {
        return prev.map((msg, idx) =>
          idx === prev.length - 1 ? { ...msg, linkPreviews: links } : msg
        );
      }
  ...
  const handleFileUpload = async (files) => {
    ...
    try {
      const replyToId = replyTo?._id || null;
      await uploadFiles(conversation._id, files, replyToId, (progress) => setUploadProgress(progress));
  ...
  const handleMediaUpload = async (files) => {
    ...
    try {
      const replyToId = replyTo?._id || null;
      await uploadMedia(conversation._id, files, replyToId, (progress) => setUploadProgress(progress));
  ...
  const handleVoiceUpload = async (audioBlob) => {
    ...
    try {
      ...
      const replyToId = replyTo?._id || null;
      await uploadVoice(conversation._id, file, replyToId, (progress) => setUploadProgress(progress));
          ...
          <>
            ...
            {isLoadingMore && (
              <div style={styles.loadMoreIndicator}>
                <div style={styles.spinner}>⟳</div>
                <span style={{ fontSize: '13px', color: '#6b7280' }}>Đang tải thêm...</span>
              </div>
            )}

            {/* No more messages indicator */}
            {!hasMore && messages.length > 0 && (
              <div style={styles.noMoreMessages}>
                <span>Đã tải hết tin nhắn</span>
              </div>
            )}
